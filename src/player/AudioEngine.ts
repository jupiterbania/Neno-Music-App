...
export class AudioEngine {
  ...
  private async loadNativeAudio(
    ...
    audio.preload = "auto";
    /*
     * Critical on Android WebView: `playsInline` prevents the system from suspending audio
     * when the app moves to the background or the screen locks, which is exactly when a media
     * app needs to keep playing. Without it, the WebView may treat the element as a video
     * needing a full-screen takeover and suspend it instead.
     *
     * `crossOrigin` is set for the media server path: without it some WebView builds refuse
     * to load the loopback URL with a CORS error even though it is same-origin from Tauri's
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const cleanup = () => {
        ...
        audio.removeEventListener("loadedmetadata", handleReadyMeta);
      ...
      /*
       * `loadedmetadata` fires before `canplay` and before `loadeddata`.
       *
       * On Android WebView with a streaming source URL (local media server or signed
       * googlevideo URL), the browser knows the format and duration once the container
       * header has been parsed — at that point it can start playing immediately. Waiting
       * for `canplay` on mobile can add hundreds of milliseconds of silent buffering
       * before the first frame is allowed to play, because the event requires the browser
       * to have buffered *enough* audio ahead, which is a heuristic that is far more
       * conservative on Android than on desktop.
       *
       * For a `sourceUrl` path there are no bytes to decode on the JS side — Rust or the
       * media server owns the body — so `loadedmetadata` is safe and sufficient: the
       * decoder is ready the moment it can read the header.
       *
       * For an in-memory blob (non-sourceUrl path), `canplay`/`loadeddata` below still
       * resolve first because the full bytes are already present, so this adds nothing
       * but also costs nothing there.
       */
      const handleReadyMeta = () => {
        if (sourceUrl) {
          cleanup();
          resolve();
        }
      ...
      audio.addEventListener("loadedmetadata", handleReadyMeta, { once: true });
      ...
      player = new window.YT!.Player(target, {
        ...
        events: {
          ...
          onStateChange: (event) => {
            ...
            // Auto-resume background playback if YouTube iframe was paused by the browser/webview
            // without user intent (e.g. phone screen turned off or app minimized)
            if (
              ...
              logInternalInfo("AudioEngine: YouTube auto-paused in background, resuming playback", {
                videoId: this.currentVideoId,
              });
              window.setTimeout(() => {
                if (!this.userPaused && this.currentVideoId && playbackOwner === this) {
                  try {
                    player.playVideo();
                  } catch (err) {
                    logInternalWarn("AudioEngine: failed to auto-resume background YouTube playback", {
                      error: err instanceof Error ? err.message : String(err),
                    });
                  }
                }
