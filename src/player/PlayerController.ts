 ...
 *
 * Raised from 5 → 10 s: mobile networks (especially 4G handoffs) can stall for several seconds
 * and a 5-second window was tight enough that a slow-to-drain buffer looked like a stream
 * failure, triggering an unnecessary reload or skip on every other track.
 */
const PREMATURE_END_TOLERANCE_SEC = 10;
...
export class PlayerController {
  ...
  private peekNextTrack(): Track | null {
    ...
    let next = this.queue.all[this.queue.currentIndex + 1] ?? null;
    if (!next && this.playbackOrderMode === "repeat-all" && this.queue.all.length > 0) {
      next = this.queue.all[0] ?? null;
    }
