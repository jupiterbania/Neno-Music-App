...
export class LibraryController {
  ...
  private async recoverSessionSilently(): Promise<void> {
    ...
    try {
      ...
      if (!(error instanceof AuthExpiredError)) {
        logInternalWarn("LibraryController silent recovery encountered non-auth error, keeping current state", {
   ...
   *
   * `status` is set to `"signed-out"` so the same sign-in prompt that a fully new user sees
   * is shown, but the library snapshot is preserved: the user can still browse their cached
   * content and the data is intact for when they sign back in.
   */
  private setSessionExpired(): void {
    ...
    this.setState({
      ...
      authPrompt: null,
      /*
       * Keep the error message soft: this is a session timeout, not a crash. The user
       * dismissed whatever error dialog surfaced before `markSessionExpired` ran — repeating it
       * as the permanent subheader under the sign-in button is redundant and alarming.
       */
      error: "Your YouTube Music session expired. Sign in again to sync and to like songs.",
      sessionConfirmedAt: null,
      // Do NOT clear `library` here. The cached snapshot is still the user's real data
      // — wiping it turns a simple re-auth into a blank slate that takes a full resync to
      // recover from, and that resync is exactly what the sign-in button will do.
    });
  }
  ...
  private async restoreSession(): Promise<void> {
    try {
       ...
       * normal outcome of a session that aged past its valid window while the app was closed.
       * `setFailure` would show an alarming dialog ("Unable to restore your YouTube Music
       * session") and leave the app in `status: "error"`, which is exactly the screenshot the
       * user filed. Routing it through `setSessionExpired` instead silently moves to the
       * sign-in prompt, keeps the cached library visible, and shows no dialog at all.
       */
      if (error instanceof AuthExpiredError) {
        this.setSessionExpired();
        return;
  ...
  async refresh(options: { suppressFailure?: boolean } = {}): Promise<void> {
    ...
    try {
      ...
      if (error instanceof AuthExpiredError) {
        ...
        /*
         * When suppressFailure is set, the caller (refreshAfterSignIn / recoverSessionSilently)
         * handles the error itself. Don't also call setFailure — that would show a duplicate
         * "Unable to load…" dialog on top of the session-expired prompt.
         *
         * When suppressFailure is NOT set (a regular background refresh), we still do NOT want
         * a `status: "error"` dialog here — `markSessionExpired` already transitioned the UI
         * to the sign-in state, which is the right outcome. Throwing the original error from a
         * suppressFailure=false refresh previously called setFailure and produced the
         * "YouTube Music sign-in or library sync failed" alert the user saw, making it look
         * like a crash rather than a routine session expiry.
         */
  ...
  private applyLibrary(library: LibrarySnapshot): void {
    this.setState({
      ...
      authPrompt: null,
      error: null,
      sessionConfirmedAt: library.account ? (this.state.sessionConfirmedAt ?? Date.now()) : null,
    });
