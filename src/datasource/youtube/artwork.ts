...
function getUrlDimension(url: string): number | null {
  const match = url.match(/[=/-](?:w|s)(\d+)/);
  return match ? parseInt(match[1], 10) : null;
}

/**
 * `size` is the requested width; omitting it keeps the original URL first if already high resolution,
 * or upgrades low-resolution thumbnails to HD.
 */
export function getArtworkUrlCandidates(url?: string, size?: number | null): string[] {
  if (!url?.trim()) return [];

  const normalized = normalizeArtworkUrl(url);
  const isVideoFallback = normalized.includes("i.ytimg.com/vi/");
  const videoId = isVideoFallback ? normalized.match(/vi\/([A-Za-z0-9_-]{11})\//)?.[1] : null;

  const dim = getUrlDimension(normalized);
  const isOriginalLarge = dim !== null && dim >= 544;

  const candidates = [
    // If a specific size was requested, try that size first
    size != null ? withYoutubeSize(normalized, size) : null,
    // If no size was asked and the original is already high-res, preserve original as first candidate
    size == null && isOriginalLarge ? normalized : null,
    // For YouTube videos, prefer 720p HD maxresdefault first, then 480p sddefault
    videoId ? `https://i.ytimg.com/vi/${videoId}/maxresdefault.jpg` : null,
    videoId ? `https://i.ytimg.com/vi/${videoId}/sddefault.jpg` : null,
    // High-resolution variants for music covers that only offered low-res thumbnails
    withYoutubeSize(normalized, 1200),
    withYoutubeSize(normalized, 800),
    withYoutubeSize(normalized, 544),
    withYoutubeSize(normalized, 400),
    normalized,
