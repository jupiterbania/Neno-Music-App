...
export class YouTubeMusicDataSource extends DataSource {
  ...
  private async getAccountCandidates(): Promise<AccountCandidate[]> {
    ...
    try {
      ...
      /*
       * If accountItems is empty, the switcher endpoint didn't return channels.
       * Do NOT call notifyAuthRejected() here as the main music session is still valid.
       */
      if (accountItems.length === 0 && this.musicCookie) {
        logInternalWarn("YouTubeMusicDataSource.getAccountCandidates signed-out stub (skipping auth rejection)");
      }
    ...
    try {
      ...
      if (!this.musicCookie) {
        ...
        if (backupCookie && typeof backupCookie === "string") {
          ...
          void invoke("save_youtube_music_cookie_direct", { cookie: backupCookie }).catch(() => {});
        }
      }
      if (!this.musicCookie) {
  ...
  async refreshSession(): Promise<boolean> {
    ...
    try {
      ...
      if (!cookie) {
        if (this.musicCookie) {
          logInternalInfo("YouTubeMusicDataSource.refreshSession keeping existing live session");
          return true;
        }
      ...
      logInternalWarn("YouTubeMusicDataSource.refreshSession error", {
        error: error instanceof Error ? error.message : String(error),
      });
      if (this.musicCookie) {
        return true;
  ...
  async signIn(
    ...
    const win = typeof window !== "undefined" ? (window as unknown as {
      ...
      AndroidMediaBridge?: {
        signInGoogle?: () => void;
        cancelSignInGoogle?: () => void;
        signOutGoogle?: () => void;
      };
    }) : undefined;

    if (isAndroid || typeof win?.AndroidMediaBridge?.signInGoogle === "function") {
      signInResult = await new Promise<SignInResult>((resolve, reject) => {
        if (!win) {
        ...
        win.__zuno_on_google_signin = async (cookie: string) => {
          ...
          try {
            ...
            const result = await invoke<SignInResult>("save_youtube_music_cookie_direct", { cookie });
            resolve(result);
          } catch (err) {
            reject(err);
          }
        ...
        if (win.AndroidMediaBridge?.signInGoogle) {
          ...
          cleanup();
          reject(new Error("Android Google sign-in bridge unavailable"));
        }
      });
    } else {
      signInResult = await invoke<SignInResult>("sign_in_youtube_music");
    }
    const { cookie, accountChanged, slotId } = signInResult;
    this.musicCookie = cookie;
    void setAppSetting("zuno:youtube-music-cookie-backup", cookie).catch(() => {});
    onStage?.("session");
  ...
  async cancelSignIn(): Promise<void> {
    if (typeof window !== "undefined") {
      const win = window as unknown as {
        __zuno_on_google_signin_cancelled?: () => void;
        AndroidMediaBridge?: { cancelSignInGoogle?: () => void };
      };
      if (win.AndroidMediaBridge?.cancelSignInGoogle) {
        win.AndroidMediaBridge.cancelSignInGoogle();
      }
      if (win.__zuno_on_google_signin_cancelled) {
        win.__zuno_on_google_signin_cancelled();
      }
    }
  ...
  async signOut(): Promise<boolean> {
    ...
    if (fallbackCookie) {
      void setAppSetting("zuno:youtube-music-cookie-backup", fallbackCookie).catch(() => {});
    } else {
      void removeAppSetting("zuno:youtube-music-cookie-backup").catch(() => {});
    }
    this.resetMusicSessionSelection();
    this.resetMusicClients();
    if (fallbackCookie) {
    ...
    const win = typeof window !== "undefined" ? (window as unknown as {
      AndroidMediaBridge?: { signOutGoogle?: () => void };
    }) : undefined;
    win?.AndroidMediaBridge?.signOutGoogle?.();
  ...
  private async hydrateArtistArtwork(artists: Artist[]): Promise<Artist[]> {
    ...
    const anyMissing = priorityArtists.some((artist) => !artist.artworkUrl);
    if (!anyMissing) return artists;

    const hydrated = await Promise.all(
      priorityArtists.map(async (artist) => {
        if (artist.artworkUrl) return artist;
        const cacheKey = this.getArtistCacheKey(artist.id);
        const cached = await getCachedJson<ArtistPage>(cacheKey);
        if (cached?.artist.artworkUrl) {
          return {
            ...artist,
            name: artist.name || cached.artist.name,
            artworkUrl: cached.artist.artworkUrl,
            subscriberCount: artist.subscriberCount || cached.artist.subscriberCount,
          };
        }

        // Bounded fetch with tight timeout so cold lookups never stall the search results response
        try {
          const timeout = new Promise<null>((resolve) => globalThis.setTimeout(() => resolve(null), 350));
          const pageArtist = await Promise.race([this.getArtistArtworkFromPage(artist.id), timeout]);
  ...
  async resolveStreamUrl(
    ...
    const order: ClientLabel[] = (usesAuthenticatedStreaming() || Boolean(this.musicCookie))
      ? ["music", "download", "web"]
      : ["download", "music", "web"];
  ...
  private static readonly BROWSE_IDS: Record<BrowseSurface, { ids: string[]; title: string }> = {
    /*
     * Personalized home feed — the same shelf layout YouTube Music shows on its own Home tab.
     * Requires a signed-in session: the feed is unique to the account and changes as the
     * algorithm learns from listening history. A signed-out request returns a generic feed,
     * which is still usable but not personalized.
     */
  ...
  async getBrowsePage(target: BrowseTarget): Promise<BrowsePage> {
    ...
    /* Home feed is personalized and changes frequently — give it its own v1 cache bucket so a
       future cache version bump on the shared browse cache does not also invalidate it, and
       so its TTL can be tuned independently in the future. */
    const cacheKey = typeof surface === "string"
      ? surface === "home"
        ? `youtube-music:browse:home:v1`
