...
import { isLinux, isMacOS, isMobileDevice } from "./platform";
...
const LOADING_SCREEN_MAX_MS = 1500;
...
export default function App() {
  ...
  }, [isMobile, playerUIState.isLyricsFullscreen]);

  const [tabs, setTabs] = useState<Tab[]>(() => {
    if (!restoredSession?.tabs || restoredSession.tabs.length === 0) {
      return [{ id: "1", view: "home" }];
    }
    const activeId = restoredSession.activeTabId ?? "1";
    return restoredSession.tabs.map((tab) => {
      // Ensure the app always opens directly on the Home screen on launch
      if (tab.id === activeId || isMobileDevice) {
        return {
          ...
          mixedSearchResults: undefined,
          searchLoading: false,
          isQueueOpen: false,
        };
      }
      return stripNavigationHistory(tab);
    });
  });
  ...
  const handleOpenBrowse = (browseTab?: string) => {
    ...
    if (existing) {
      ...
      if (browseTab && existing.browseTab !== browseTab) {
        setTabs((prevTabs) =>
          prevTabs.map((tab) =>
            tab.id === existing.id
              ? { ...tab, browseTab, title: browseTab === "downloads" ? "Downloads" : "Browse" }
              : tab,
          ));
      }
    ...
    setTabs((prevTabs) => [
      ...prevTabs,
      {
        id: newId,
        view: "browse",
        title: browseTab === "downloads" ? "Downloads" : "Browse",
        browseTab,
      },
    ]);
    setActiveTabId(newId);
 ...
 */
  useEffect(() => {
    if (isMobile || miniPlayerEnabled) return;
    void destroyMiniPlayerWindow();
  }, [miniPlayerEnabled, isMobile]);
    ...
    */}
    <div
      className={cn(
        ...
        isMobile
          ? "h-dvh rounded-none border-0 ring-0"
          : cn(
              "h-screen rounded-[var(--window-radius)]",
    ...
    /> */}
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        ...
        <Layout
          ...
          hideSidebar={isMobile || playerUIState.isLyricsFullscreen}
        ...
        {isMobile ? (
          <>
            <div className="fixed bottom-0 inset-x-0 z-40 flex flex-col pointer-events-none">
              <div className="pointer-events-auto w-full px-2 pb-1.5">
                <MobilePlayerBar onOpenNowPlaying={() => setIsMobileNowPlayingOpen(true)} />
              </div>
