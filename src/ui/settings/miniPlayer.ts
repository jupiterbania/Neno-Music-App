...
import { isAndroid, isMobileDevice } from "../platform";
...
function readMiniPlayerEnabled() {
  if (isMobileDevice || isAndroid) return false;
...
export async function hydrateMiniPlayerSettings() {
  if (isMobileDevice || isAndroid) return;
...
async function createMiniPlayerWindow(): Promise<WebviewWindow | null> {
  if (isMobileDevice || isAndroid) return null;
...
export function ensureMiniPlayerWindow(): Promise<WebviewWindow | null> {
  if (isMobileDevice || isAndroid) return Promise.resolve(null);
...
export function destroyMiniPlayerWindow(): Promise<void> {
  if (isMobileDevice || isAndroid) return Promise.resolve();
