...
export function AlbumCardSkeleton({ delayMs = 0 }: { delayMs?: number }) {
  return (
    <div className="flex w-full flex-col gap-2 rounded-xl p-1.5 sm:p-2">
      <SkeletonBlock className="aspect-square w-full rounded-xl sm:rounded-2xl" delayMs={delayMs} />
      <SkeletonBlock className="h-3.5 w-[78%] rounded-md" delayMs={delayMs} />
      <SkeletonBlock className="h-3 w-[48%] rounded-md" delayMs={delayMs} />
    </div>
  );
}
