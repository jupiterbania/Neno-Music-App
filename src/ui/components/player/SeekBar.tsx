...
const SEEK_SLIDER_STACKED = [
  "h-1.5 w-full cursor-pointer appearance-none rounded-full bg-transparent",
  "disabled:cursor-default disabled:opacity-50",
  "focus-visible:outline-none",
  // Track: filled with clean white, unfilled with subtle semi-transparent white
  "[&::-webkit-slider-runnable-track]:h-1 [&::-webkit-slider-runnable-track]:rounded-full",
  "[&::-webkit-slider-runnable-track]:bg-[linear-gradient(to_right,rgba(255,255,255,0.95)_var(--slider-progress),rgba(255,255,255,0.2)_var(--slider-progress))]",
  "[&::-moz-range-track]:h-1 [&::-moz-range-track]:rounded-full [&::-moz-range-track]:bg-white/20",
  "[&::-moz-range-progress]:h-1 [&::-moz-range-progress]:rounded-full [&::-moz-range-progress]:bg-white",
  // Thumb: visible circle matching YouTube Music mobile
  "[&::-webkit-slider-thumb]:size-3 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:rounded-full",
  "[&::-webkit-slider-thumb]:-mt-1 [&::-webkit-slider-thumb]:bg-white [&::-webkit-slider-thumb]:shadow-[0_2px_4px_rgba(0,0,0,0.5)]",
  "[&::-webkit-slider-thumb]:active:scale-125 [&::-webkit-slider-thumb]:transition-transform",
  "[&::-moz-range-thumb]:size-3 [&::-moz-range-thumb]:rounded-full [&::-moz-range-thumb]:bg-white",
].join(" ");
...
export function SeekBar({ layout = "horizontal", className }: SeekBarProps = {}) {
  ...
  if (layout === "stacked") {
    return (
      <div className={cn("group/seek flex w-full flex-col gap-1.5", className)}>
        <input
          type="range"
          min="0"
          max={duration || 100}
          ...
          disabled={isDisabled}
          className={SEEK_SLIDER_STACKED}
          style={{
            "--slider-progress": `${duration > 0 ? (currentTime / duration) * 100 : 0}%`,
          } as React.CSSProperties}
          aria-label="Seek"
        />
        <div className="flex w-full items-center justify-between text-xs tabular-nums text-white/60 px-0.5 select-none">
  ...
  return (
    <div className={cn("group/seek flex w-full items-center gap-2.5", className)}>
