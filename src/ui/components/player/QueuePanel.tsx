...
const QueueRow = memo(function QueueRow({
  ...
  const row = (
    <div
      ...
      className={cn(
        "group/queue-item relative flex items-center rounded-xl transition-colors hover:bg-card/70 active:bg-card",
        // The pointer handler writes --drag-translation; this is what renders the lift.
        "[transform:translateY(var(--drag-translation,0px))]",
        collapsed ? "justify-center" : isMobile ? "gap-2.5 px-3 py-2" : "gap-1",
    ...
    >
      <button
        type="button"
        className={cn(
          "flex min-w-0 items-center rounded-lg text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring",
          collapsed ? "p-1.5" : isMobile ? "flex-1 gap-3" : "flex-1 gap-2.5 p-1.5",
        )}
        ...
        <span className="relative shrink-0">
          <TrackArtwork
            className={cn(
              ...
              isMobile ? "size-11 ring-1 ring-white/10" : (collapsed ? "size-11 rounded" : "size-10 rounded"),
            )}
            size={isMobile ? 120 : (collapsed ? 44 : 40)}
            artworkUrl={track.artworkUrl}
            iconSize={isMobile ? 20 : (collapsed ? 20 : 18)}
          />
          <span
            className={cn(
      ...
      {!collapsed && isMobile && track.durationSec ? (
        <span className="mr-1 shrink-0 text-xs tabular-nums text-muted-foreground">
...
export function QueuePanel({ onClose }: QueuePanelProps) {
  ...
  const handleTrackPointerDown = (
    ...
    trackItem.style.setProperty("will-change", "transform");
  };
  ...
  const sectionLabel = (label: string, count: number) =>
    collapsed ? (
      ...
      <div className={cn("flex items-baseline justify-between gap-2 pb-1.5 pt-2.5", isMobile ? "px-4" : "px-2 pb-1.5 pt-1")}>
  ...
  return (
    ...
    >
      <header
        className={cn(
          "sticky top-0 z-10 flex shrink-0 items-center gap-1",
          isMobile
            ? "bg-background/95 px-4 py-3 backdrop-blur-xl border-b border-white/5 shadow-sm"
            : cn("bg-card", collapsed ? "flex-col px-2 py-2" : "px-3 py-2.5"),
        )}
      >
        {!isMobile && (
          <Tooltip
        ...
        {!collapsed && upcomingCount > 0 && (
          <div className="flex items-center gap-1">
            <Tooltip content="Shuffle what's next">
              <button
                type="button"
                className={cn(ICON_BUTTON, isMobile && "size-8 text-foreground/80 hover:bg-white/10 active:scale-95")}
                onClick={() => playerController.shuffleUpcomingQueue()}
              >
                <ShuffleIcon size={isMobile ? 18 : 16} aria-hidden="true" />
            ...
            <Tooltip content="Shuffle the whole playlist">
              ...
              >
                <ShuffleActiveIcon size={isMobile ? 18 : 16} aria-hidden="true" />
            ...
            <Tooltip content="Save the queue as a playlist">
              <button
                ...
                className={cn(
                  ICON_BUTTON,
                  isMobile && "size-8 hover:bg-white/10 active:scale-95",
            ...
            <Tooltip content="Clear the queue">
              <button
                type="button"
                className={cn(ICON_BUTTON, isMobile && "size-8 hover:bg-white/10 active:scale-95", "hover:text-primary")}
                onClick={() => playerController.clearUpcomingQueue()}
              >
                <TrashIcon size={isMobile ? 18 : 16} aria-hidden="true" />
      ...
      {currentTrack && (
        <div
          className={cn(
            ...
            isMobile
              ? "mx-4 my-2.5 gap-3.5 rounded-2xl bg-white/[0.06] p-3 border border-white/10 shadow-lg backdrop-blur-md"
              : cn(
        ...
        >
          <span className="relative shrink-0">
            <TrackArtwork
              className={cn(
                ...
                isMobile
                  ? "size-14 ring-1 ring-white/20"
                  : (collapsed ? "size-11 rounded" : "size-10 rounded ring-1 ring-primary/60"),
              )}
              size={isMobile ? 120 : (collapsed ? 44 : 40)}
              artworkUrl={currentTrack.artworkUrl}
              iconSize={isMobile ? 24 : (collapsed ? 20 : 18)}
            />
            {isPlaying && (
              <span
                className="absolute inset-0 grid place-items-center rounded-xl bg-background/60 backdrop-blur-[2px]"
                aria-hidden="true"
              >
                <MusicVisualizer
                  bars={4}
          ...
          {!collapsed && (
            <span className="flex min-w-0 flex-1 flex-col">
              ...
              <span className={cn("truncate font-semibold text-foreground", isMobile ? "text-base" : "text-sm font-medium")}>
                {currentTrack.title}
              </span>
              <ArtistLinks
                className={cn("truncate text-muted-foreground", isMobile ? "text-xs mt-0.5" : "text-xs")}
        ...
        collapsed ? null : (
          <div className="flex flex-1 flex-col items-center justify-center p-8 text-center">
            ...
            <p className="mt-1 text-xs text-muted-foreground/60">
              Songs you add with &ldquo;Play next&rdquo; will appear here.
            </p>
          </div>
        )
      ) : (
        <div className={cn("flex flex-col gap-1", isMobile ? "px-2 pb-24" : (collapsed ? "px-1.5 pb-2" : "px-2 pb-2"))}>
          {manual.length > 0 && (
            <>
