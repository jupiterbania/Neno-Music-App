...
export function UpdateToast({ update, onDismiss }: UpdateToastProps) {
  ...
  const releaseLabel = update.canInstall ? "View" : "Download";
  ...
  const install = async () => {
    ...
    try {
      ...
      setError("Installation failed. Please try again later.");
      setInstalling(false);
    }
  };
    ...
    >
      ...
      <div className="flex shrink-0 items-center gap-1.5">
        ...
        <a
          ...
          aria-label={`Open version ${update.version} update details`}
          onClick={(e) => {
            e.preventDefault();
