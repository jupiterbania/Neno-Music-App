...
import { CheckActiveIcon, CheckIcon, DislikeActiveIcon, DislikeIcon, DownloadIcon, HeartActiveIcon, HeartIcon, ListIcon, MenuDotsIcon, PlaylistAddIcon, PlayActiveIcon } from "@/ui/icons";
      ...
      <span
        ...
        className={cn(
          ...
          status === "ready"
            ...
            : "hidden md:grid text-muted-foreground opacity-0 hover:bg-background hover:text-foreground group-hover/row:opacity-100 focus:opacity-100",
          (status === "queued" || status === "downloading") && "grid opacity-100",
          status === "failed" && "grid text-destructive opacity-100",
        )}
        onClick={(event) => {
  ...
  return (
    ...
    >
      ...
      */}
      ...
      {/* Trailing action icons group: Heart, Download, Quick actions, Trailing content, 3 dots menu */}
      <div className="flex shrink-0 items-center gap-0.5 sm:gap-1">
        {showRating && <RatingActions track={track} />}
        {showDownload && <DownloadAction track={track} />}

        {(onQuickAddToQueue || onQuickAdd) && (
          <span className="hidden md:flex shrink-0 items-center gap-0.5">
            {onQuickAddToQueue && (
              <QuickAction
        ...
        {/* Touch-friendly More options button on mobile */}
        {onContextMenu && (
          <span
            role="button"
            tabIndex={0}
            aria-label={`More options for ${track.title}`}
            className="flex size-8 shrink-0 items-center justify-center rounded-full text-muted-foreground transition-colors hover:text-foreground active:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring md:hidden"
            onClick={(event) => {
              event.stopPropagation();
            ...
            onKeyDown={(event) => {
              ...
              event.preventDefault();
              event.stopPropagation();
              handleContextMenu(event as never);
            }}
          >
            <MenuDotsIcon size={16} aria-hidden="true" className="rotate-90" />
          </span>
        )}
      </div>
